...
async function buildAPI(html, jar) {
  ...
  return [ctx, defaultFuncs, { refreshFb_dtsg }];
}
  ...
  try {
    ...
    if (appState) {
      ...
      if (Array.isArray(appState)) {
        cookieStrings = appState.map(c => `${c.name || c.key}=${c.value}`);
      } else if (typeof appState === 'string') {
        cookieStrings = appState.split(';').map(c => c.trim()).filter(Boolean);
      } else {
        throw new Error("Invalid appState format: must be an array or a header string");
      }

      cookieStrings.forEach(cookieStr => {
        const domain = ".facebook.com";
        const expires = new Date().getTime() + 1000 * 60 * 60 * 24 * 365;
        const str = `${cookieStr}; expires=${expires}; domain=${domain}; path=/;`;
...
async function login(cookie, options, callback) {
  ...
  const loginWs3 = () => {
    loginHelper(cookie, { relogin: loginWs3 }, (loginError, loginApi) => {
      if (loginError) {
